...
class ECommerceTransactions:
    def __init__(self):
        ...
        self.transactions = np.array([
            np.arange(1000),  # transaction_id
            np.random.randint(1, 101, 1000),  # user_id
            np.random.randint(1, 501, 1000),  # product_id
            np.random.randint(1, 11, 1000),  # quantity
            np.random.uniform(10, 1000, 1000),  # price
            [int(ts.timestamp()) for ts in timestamps]  # timestamp in seconds
        ]).T
    ...
    def total_revenue(self):
        ...
        """
        return np.sum(self.transactions[:, 3] * self.transactions[:, 4])
    ...
    def convert_price_to_int(self):
        ...
        rounded_prices = np.round(self.transactions[:, 4])
    ...
    def increase_prices(self, percentage):
        ...
        """
        self.transactions[:, 4] *= (1 + percentage / 100)
        return self.transactions
    ...
    def filter_transactions(self):
        ...
        """
        return self.transactions[self.transactions[:, 3] > 1]

    def revenue_comparison(self, timestamp1, timestamp2):
        """
        Compare the revenue from two different time periods.
        """
        mask1 = self.transactions[:, 5] < timestamp1
        mask2 = (self.transactions[:, 5] >= timestamp1) & (self.transactions[:, 5] < timestamp2)
        revenue1 = np.sum(self.transactions[mask1][:, 3] * self.transactions[mask1][:, 4])
        revenue2 = np.sum(self.transactions[mask2][:, 3] * self.transactions[mask2][:, 4])
    ...
    def user_transactions(self, user_id):
        ...
        """
        return self.transactions[self.transactions[:, 1] == user_id]
    ...
    def date_range_transactions(self, start_date, end_date):
        ...
        return self.transactions[
            (self.transactions[:, 5] >= start_timestamp) & (self.transactions[:, 5] < end_timestamp)]
    ...
    def top_products(self):
        ...
        product_revenue = np.bincount(self.transactions[:, 2].astype(int),
                                      weights=self.transactions[:, 3] * self.transactions[:, 4])
        top_5_products = np.argsort(product_revenue)[-5:]
        return self.transactions[np.isin(self.transactions[:, 2], top_5_products)]

    def get_readable_dates(self):
        """Convert timestamps to readable date strings"""
        return [datetime.fromtimestamp(ts).strftime('%Y-%m-%d') for ts in self.transactions[:, 5]]
...
def main():
    ...
    assert np.all(increased_prices[:, 4] >= ecommerce_data.transactions[:, 4]), "Prices were not increased correctly"
    ...
    date_range_transactions = ecommerce_data.date_range_transactions("2023-06-01", "2023-07-01")
    ecommerce_data.print_array(
        date_range_transactions[:5],
        message="Sample of transactions in date range:"
    )
    assert date_range_transactions.shape[1] == ecommerce_data.transactions.shape[
        1], "Shape of date range transactions is incorrect"

    top_products = ecommerce_data.top_products()
    ecommerce_data.print_array(
        top_products[:5],
        message="Sample of transactions for top 5 products:"
    )
    assert top_products.shape[1] == ecommerce_data.transactions.shape[1], "Shape of top products transactions is incorrect"
